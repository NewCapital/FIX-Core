...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zfixTracker = unique_ptr<CzFIXTracker>(new CzFIXTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZFIX_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZFIX_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
