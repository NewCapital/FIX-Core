...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total fix balance of the wallet (excluding zerocoins)\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zfixObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    //zfixObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    //obj.push_back(Pair("zFIXsupply", zfixObj));
...
UniValue spork(const UniValue& params, bool fHelp)
{
    if (params.size() == 1 && params[0].get_str() == "show") {
        ...
        for (int nSporkID = SPORK_START; nSporkID <= SPORK_FIX_END; nSporkID++) {
            if (nSporkID > SPORK_END && nSporkID < SPORK_FIX_START) nSporkID = SPORK_FIX_START;
...
#ifdef ENABLE_WALLET
        // Case 1: FIX address and we have full public key:
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"fixaddress\" \"signature\" \"message\"\n"
