...
#include "ui_zfixcontroldialog.h"
...
std::set<std::string> ZFIXControlDialog::setSelectedMints;
std::set<CMintMeta> ZFIXControlDialog::setMints;

bool CZFIXControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZFIXControlDialog::COLUMN_DENOMINATION || column == ZFIXControlDialog::COLUMN_VERSION || column == ZFIXControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZFIXControlDialog::ZFIXControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZFIXControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZFIXControlDialog::~ZFIXControlDialog()
{
    delete ui;
}

void ZFIXControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFIXControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZFIXControlWidgetItem* itemDenom(new CZFIXControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZFIXControlWidgetItem *itemMint = new CZFIXControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zFIX seed used to mint this zFIX is not the same as currently hold in the wallet";
            else
...
void ZFIXControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFIXControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZFIX_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFIXControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZFIXControlDialog::GetSelectedMints()
{
...
void ZFIXControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
