...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FIX);
    ...
    if (!settings.contains("nAnonymizeFIXAmount"))
        settings.setValue("nAnonymizeFIXAmount", 1000);

    nAnonymizeFIXAmount = settings.value("nAnonymizeFIXAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFIXAmount"))
        SoftSetArg("-anonymizefixamount", settings.value("nAnonymizeFIXAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fix.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFIXAmount:
            return QVariant(nAnonymizeFIXAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeFIXAmount:
            nAnonymizeFIXAmount = value.toInt();
            settings.setValue("nAnonymizeFIXAmount", nAnonymizeFIXAmount);
            emit anonymizeFIXAmountChanged(nAnonymizeFIXAmount);
            break;
        case CoinControlFeatures:
