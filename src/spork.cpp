...
// FIX: on startup load spork values from previous session if they exist in the sporkDB
void LoadSporksFromDB()
{
    for (int i = SPORK_START; i <= SPORK_FIX_END; ++i) {
        if (i > SPORK_END && i < SPORK_FIX_START) i = SPORK_FIX_START;
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_FIX_01_ENABLE_MASTERNODE_TIERS) r = SPORK_FIX_01_ENABLE_MASTERNODE_TIERS_DEFAULT;
        if (nSporkID == SPORK_FIX_02_MIN_STAKE_AMOUNT) r = SPORK_FIX_02_MIN_STAKE_AMOUNT_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_FIX_01_ENABLE_MASTERNODE_TIERS") return SPORK_FIX_01_ENABLE_MASTERNODE_TIERS;
    if (strName == "SPORK_FIX_02_MIN_STAKE_AMOUNT") return = SPORK_FIX_02_MIN_STAKE_AMOUNT_DEFAULT;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_FIX_01_ENABLE_MASTERNODE_TIERS) return "SPORK_FIX_01_ENABLE_MASTERNODE_TIERS";
    if (id == SPORK_FIX_02_MIN_STAKE_AMOUNT) return = SPORK_FIX_02_MIN_STAKE_AMOUNT_DEFAULT;
