...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZFIXStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZFIXStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZFIXStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight + 1, blockValue, 0, fZFIXStake);
    ...
    if (hasPayment) {
        if (fProofOfStake) {
            ...
            if (pindexPrev->nHeight < ((Params().NetworkID() == CBaseChainParams::MAIN)?20000:1000)) nDevReward = 1 * COIN;
            if (nDevReward > 0) {
                CTxDestination destination = CBitcoinAddress(Params().DevAddress()).Get();
                if (pindexPrev->nHeight >= 3000 && Params().NetworkID() == CBaseChainParams::TESTNET) destination = CBitcoinAddress(Params().DevAddressNew()).Get();
