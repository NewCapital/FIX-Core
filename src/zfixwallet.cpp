...
CzFIXWallet::CzFIXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfix seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFIXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFIXSeed_deprecated()) {
                    LogPrintf("%s: Updated zFIX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfix seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfix wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFIXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFIXCount(nCountLastUsed);
    else if (!walletdb.ReadZFIXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFIXWallet::Lock()
{
    seedMaster = 0;
}

void CzFIXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFIXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFIX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFIXWallet::LoadMintPoolFromDB()
{
...
void CzFIXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFIXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFIXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfixTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFIXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFIX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zfixTracker which also adds to database
    pwalletMain->zfixTracker->Add(dMint, true);
...
void CzFIXWallet::SeedToZFIX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzFIXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFIXWallet::GenerateDeterministicZFIX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFIXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFIXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzFIXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
