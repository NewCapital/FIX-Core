...
#if defined(NDEBUG)
#error "FIX cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ufix) are considered zero fee (for relaying and mining)
...
bool isMasternodeCollateral(CAmount nValue) {
  if (IsSporkActive(SPORK_FIX_01_ENABLE_MASTERNODE_TIERS)) {
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zFIX spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zFIX is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zFIX spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zFIX without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zFIX spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zFIX spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zfix");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zFIX mints are not already known
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZFIXStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from FIX
...
{
    RenameThread("fix-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZFIXMinted()
{
...
void RecalculateZFIXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zFIX supply
...
bool RecalculateFIXSupply(int nHeightStart)
{
...
{
    // FIX: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZFIXSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZFIXMinted();
        RecalculateZFIXSpent();
        RecalculateFIXSupply(Params().Zerocoin_StartHeight());
    }

    //Track zFIX money supply in the block index
    if (!UpdateZFIXSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zFIX supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zFIXSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
	...
	if (pindex->pprev->nHeight >= 3000 && Params().NetworkID() == CBaseChainParams::TESTNET) destination = CBitcoinAddress(Params().DevAddressNew()).Get();
        ...
        if (!DevPaid) {
            return state.DoS(0, error("ConnectBlock(FIX): no dev reward"), REJECT_INVALID, "no-dev-reward");
        }
      }
    }
    // FIX>
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert FIX to zFIX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (IsSporkActive(SPORK_FIX_02_MIN_STAKE_AMOUNT &&
              nHeight >= (Params().NetworkID() == CBaseChainParams::MAIN? 250000 : 192500) )) {
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zFIX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zFIX serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zFIX stake block is less than allowed start height", __func__);

    if (CZFIXStake* zFIX = dynamic_cast<CZFIXStake*>(stake)) {
        CBlockIndex* pindexFrom = zFIX->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zFIX stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zFIX stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zFIX->GetValue()));
        if (nChecksum200 != zFIX->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zFIX->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZFIX() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zFIX fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> fixInputs;
        std::vector<CTxIn> zFIXInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zFIXInputs.push_back(stakeIn);
            }else{
                fixInputs.push_back(stakeIn);
            }
        }
        const bool hasFIXInputs = !fixInputs.empty();
        const bool hasZFIXInputs = !zFIXInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasFIXInputs)
                    ...
                    for (const CTxIn& fixIn : fixInputs){
                        if(fixIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : fixInputs) {
            ...
            // Now that this loop if completed. Check if we have zFIX inputs.
            if(hasZFIXInputs){
                for (const CTxIn& zFIXInput : zFIXInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zFIXInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zfix");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zFIX mints and %d zFIX spends\n", __func__, nMints, nSpends);
...
bool CVerifyDB::VerifyDB(CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    CBlockIndex* pindex = chainActive.Tip();
    int fixLastCheckpoint = 0;

    if (pindex)
      fixLastCheckpoint = Checkpoints::GetClosestCheckpoint(pindex->nHeight);

    if (fixLastCheckpoint) {
      LogPrintf("Validating Last Checkpoint (current heigh=%s last checkpoint heigh=%s)...", pindex->nHeight, fixLastCheckpoint);
      if (!Checkpoints::CheckBlock(fixLastCheckpoint,chainActive[fixLastCheckpoint]->GetBlockHash())) {
        LogPrintf("FAILED !!!\n");
        return error("VerifyDB() : *** Checkpoint validation at block %s FAILED", fixLastCheckpoint);
      } else {
        LogPrintf("Passed\n");
      }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // FIX: We use certain sporks during IBD, so check to see if they are
