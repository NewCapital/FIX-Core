...
public:
    ...
    bool WriteZFIXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZFIXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZFIXSeed_deprecated(uint256& seed);
    bool EraseZFIXSeed();
    bool EraseZFIXSeed_deprecated();

    bool WriteZFIXCount(const uint32_t& nCount);
    bool ReadZFIXCount(uint32_t& nCount);
