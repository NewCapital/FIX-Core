...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZFIXStake(spend));
    } else {
        ...
        CFIXStake* FIXInput = new CFIXStake();
        FIXInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(FIXInput);
    }
