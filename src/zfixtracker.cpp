...
CzFIXTracker::CzFIXTracker(std::string strWalletFile)
{
...
CzFIXTracker::~CzFIXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzFIXTracker::Init()
{
...
bool CzFIXTracker::Archive(CMintMeta& meta)
{
...
bool CzFIXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzFIXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzFIXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzFIXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzFIXTracker::GetSerialHashes()
{
...
CAmount CzFIXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzFIXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzFIXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzFIXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzFIXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzFIXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzFIXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzFIXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzFIXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzFIXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzFIXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzFIXWallet* zFIXWallet)
{
    bool iszFIXWalletInitialized = (NULL != zFIXWallet);
    CMintMeta meta;
    ...
    if (! iszFIXWalletInitialized)
        zFIXWallet = new CzFIXWallet(strWalletFile);
    meta.isSeedCorrect = zFIXWallet->CheckSeed(dMint);
    if (! iszFIXWalletInitialized)
        delete zFIXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzFIXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzFIXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzFIXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzFIXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzFIXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzFIXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzFIXWallet* zFIXWallet = new CzFIXWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zFIXWallet);
        }
        delete zFIXWallet;
        LogPrint("zero", "%s: added %d dzfix from DB\n", __func__, listDeterministicDB.size());
    }
